...
def crear_transaccion(transaccion: dict):
    conn=get_connection()
    cursor=conn.cursor()
    cursor.execute("INSERT INTO transacciones (descripcion,monto) VALUES (%s,%s)",(transaccion["descripcion"],transaccion["monto"]))
    conn.commit()
    cursor.close()
    conn.close()
    return {
...
def modificar_transaccion(indice:int,new_data:dict):
    conn=get_connection()
    cursor=conn.cursor()
    cursor.execute("UPDATE transacciones SET descripcion=%s,monto=%s WHERE id=%s",(new_data["descripcion"],new_data["monto"], indice))
    conn.commit()
    cursor.close()
    conn.close()
    return {
